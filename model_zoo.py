...
def spvnas_specialized(net_id, pretrained=True, **kwargs):
    ...
    if pretrained:
        init = torch.load(
            download_url(
                url_base + net_id + "/init", model_dir=".torch/spvnas_specialized/%s/" % net_id
            ),
...
def spvnas_supernet(net_id, pretrained=True, **kwargs):
    ...
    net_config = json.load(
        open(
            download_url(
                url_base + net_id + "/net.config", model_dir=".torch/spvnas_supernet/%s/" % net_id
            )
        )
    )
    ...
    if pretrained:
        init = torch.load(
            download_url(
                url_base + net_id + "/init", model_dir=".torch/spvnas_supernet/%s/" % net_id
            ),
            map_location="cuda:%d" % dist.local_rank() if torch.cuda.is_available() else "cpu",
        )["model"]
        model.load_state_dict(init)
...
def minkunet(net_id, pretrained=True, **kwargs):
    ...
    net_config = json.load(
        open(
            download_url(
                url_base + net_id + "/net.config", model_dir=".torch/minkunet/%s/" % net_id
            )
        )
    )

    model = MinkUNet(num_classes=net_config["num_classes"], cr=net_config["cr"]).to(
        "cuda:%d" % dist.local_rank() if torch.cuda.is_available() else "cpu"
    )
