...
class KITTI360(object):
    def __init__(self, data_dir, seq=0, cam=0):
        ...
        # camera poses 
        sequence_dir = '%s/data_poses/2013_05_28_drive_%04d_sync/' % (data_dir, seq)
        camera_dir = '%s/data_2d_raw/2013_05_28_drive_%04d_sync/' % (data_dir, seq)
        self.pose_file = os.path.join(sequence_dir, 'cam%d_to_world.txt' % cam)
        self.image_dir = '%simage_%02d/data_rect/' % (camera_dir, cam)
...
if __name__=='__main__':
    dset = KITTI360('/scratch-second/KITTI360')
    for i in range(10):
        dset[i]
