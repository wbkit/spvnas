...
import yaml
import sys
from core.datasets.devkits.commons.loadCalibration import loadCalibrationCameraToPose
    ...
    def __init__(self):
        ...
        for frame, pose in zip(frames, poses):
            ...
            if self.cam_id == 0 or self.cam_id == 1:
                self.cam2world[frame] = np.matmul(
                    np.matmul(pose, self.camToPose), np.linalg.inv(self.R_rect)
                )
            # fisheye cameras
...
if __name__ == "__main__":
    ...
    print(f" Using KITTI360 dataset from {kitti360Path}")
    ...
    for frame in camera.frames:
        ...
        for k, v in annotation3D.objects.items():
            if len(v.keys()) == 1 and (-1 in v.keys()):  # show static only
                obj3d = v[-1]
                if not id2label[obj3d.semanticId].name == "car":  # show static cars only
                    continue
                camera(obj3d, frame)
                ...
                for line in obj3d.lines:
                    v = [
                        obj3d.vertices[line[0]] * x + obj3d.vertices[line[1]] * (1 - x)
                        for x in np.arange(0, 1, 0.01)
                    ]
                    uv, d = camera.project_vertices(np.asarray(v), frame)
                    mask = np.logical_and(np.logical_and(d > 0, uv[0] > 0), uv[1] > 0)
                    mask = np.logical_and(
                        np.logical_and(mask, uv[0] < image.shape[1]), uv[1] < image.shape[0]
                    )
